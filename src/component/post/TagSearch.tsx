    ...
    >
      ...
      >
        ...
        <Typography variant="subtitle1" sx={{ mb: "12px" }}>
          Feel free to take any tags!{" "}
        </Typography>
        {posts.map((post, index) => (
          <Box key={index} sx={{ display: "inline-block", marginRight: "8px" }}>
            <ColoredChips keywords={post.tags} setSelectedWord={(word) => setKeyword(word)} />
          </Box>
        ))}
      </Box>
      ...
      <Box>
        ...
        {posts.length > postsPerPage && (
          <Box sx={{ display: "flex", justifyContent: "center", marginTop: "20px" }}>
            {Array.from(
              { length: Math.ceil(currentPosts.length / postsPerPage) },
              (_, index) => index + 1
            ).map((pageNumber) => {
